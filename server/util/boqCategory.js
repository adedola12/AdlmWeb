 ...
 *
 * The category set depends on the source tool/productKey:
 *   - QUIV (revit, revit-materials, planswift, etc.)  -> Substructure | Frames | Superstructure
 *   - Revit MEP (revitmep, mep, revit-mep)            -> HVAC | Plumbing | Electrical
 *
 * Items that don't match any rule fall into "Uncategorized" so the user can see and override.
 */
...
/**
 * Derive a category for a single item. Returns one of the productKey's category names
 * or UNCATEGORIZED. Pass the existing item.category through if already set and valid.
 */
