...
export interface AdlmConfig {
  ...
  /**
   * Where AWS Support replies, AWS Health notices and the daily-report
   * watchdog go (lib/adlm-ops-alerts-stack.ts). Must be an inbox somebody
   * reads every day, and each stack's confirmation email must be clicked
   * within three days of deploying it.
   */
  opsAlertEmail: string;

  /**
   * The scheduled-job log group, read by the daily-report watchdog. Its name
   * is generated by CloudFormation, so it is written down here rather than
   * derived.
   */
...
export const config: AdlmConfig = {
  ...
  // subscribed by hand on 2026-09-16. Check with
  //   aws sns list-subscriptions-by-topic --topic-arn <AlarmTopic arn>
  alarmEmail: "admin@adlmstudio.net",
