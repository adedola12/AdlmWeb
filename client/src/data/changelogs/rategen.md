...
- **The MEP light fittings in 2.7.0 were priced at up to 48 times what the fitting costs.** This is the most serious thing in this release and it was our mistake. The lighting rates came from a priced bill whose electrical section was billed far above the cost of the goods, and we published them without checking them against what the fittings actually sell for. A 12W LED ceiling fitting that sells for about ₦3,500 was in your library at ₦169,000. A 7W LED spot that sells for ₦2,500 was at ₦110,000. Every installed light fitting has been rebuilt as the supply price, plus 10% for flex, connectors and fixings, plus the electrician and mate time to fix and connect it, using the labour rates already in your library. **If you priced any job with lighting between the 2.7.0 release and now, those rates were too high and the bill will need revisiting.** A complete lighting point with a 600 x 600mm LED panel and switch was ₦111,900 and is now ₦75,900.
- **Point wiring has been rebuilt from its components too**, though this one was closer to right than it looked. There is no supply price to check point wiring against, since it is conduit, cable, boxes and time, so we built it: 20mm PVC conduit and single core copper at current supplier prices, plus an electrician, mate and labourer at your library's own day rates. The lighting point came out at ₦30,500 against the ₦35,000 we had, and the socket point at ₦35,000 against ₦47,500. Both are updated so the whole electrical section now rests on one set of evidence.
- **Four materials were priced at zero, and a roofing sheet had been corrupted.** A material at zero does not fail loudly. It quietly under-prices every rate that uses it and the rate still looks like a rate. The roofing row is the clearest case: someone editing "0.55mm (24SWG) sheet, coloured" typed an extra letter into the name and lost the price, so the correctly named row read zero while the real price sat under the typo. That is recovered. Bitumen 60/70 and two Peacock paints are now priced from their nearest neighbour in the same category, and are marked as estimates rather than quotations.
...
- **Open a rate's components straight from its breakdown.** When a rate looks wrong it is nearly always one line that is wrong, and finding it used to mean reading the breakdown, remembering the name, opening the library and hunting through 600 rows. Click a component name in any rate breakdown and the library opens filtered to it, ready to edit. This works in every section. Not every line is a link: totals, sub-totals and "add for waste" rows are arithmetic rather than library items, and a few components are labelled in a way that does not match any library row yet, so those stay as plain text.
...
- **Price by state.** Your ADLM profile now asks for a state rather than a geopolitical zone, covering all 36 and the FCT. It is set in one place, on your profile on the website, and RateGen picks it up on your next sign-in. There is no second picker in the app on purpose: two places to set the same thing means two answers that can disagree, with nothing on screen to say which won. The library still tells you where its prices came from. Be aware of what this does and does not do yet: prices are evidenced at zone level, so Kano and Katsina read the same today because they are both North West. What changes is that your state can now be priced on its own when we have evidence for it, without you doing anything.
- **Set your own prices, on the website, for your location.** On the RateGen page, click any price in Master Materials or Master Labour and type what you actually pay. Your figure is used by RateGen, QUIV and HERON from your next sign-in, and reset puts the published price back. It applies to the state on your profile, so a correction you make for Kano does not follow you to a job in Lagos.
...
- **HVAC and fire protection rates, and these also reach you without updating.** Ten more in Carbon and Others: split air conditioners at 1HP and 2HP, ceiling and duct extractor fans, ceiling fan, smoke and heat detector, alarm sounder, 8 zone alarm panel, extinguisher, and hydrant with hose reel. Carbon and Others now carries 26 rates in total. The two air conditioner rates include the 20A isolator and the electrician time to connect it, because the unit price on its own does not cover getting it running.
