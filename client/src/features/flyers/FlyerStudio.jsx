// FlyerStudio — the orchestrator for the ADLM Flyer Engine (port of the NIQS
// engine's App.jsx). Owns the flyer state, the server-backed store, the
// offscreen 1080×1350 export node, and ALL export/save logic; the form, canvas,
// export bar, and saved-list are presentational children.
...
import { defaultFlyer, normalizeFlyer, TEMPLATES } from "./lib/defaults.js";
...
const PREVIEW_H = Math.round(FLYER_H * SCALE);
...
export default function FlyerStudio({ accessToken }) {
  ...
  const [packVariant, setPackVariant] = useState(null); // transient template for pack capture
  const [showSaved, setShowSaved] = useState(false);
  const [busy, setBusy] = useState(null); // 'png' | 'pdf' | 'pack'
  ...
  const templateLabel =
    TEMPLATES.find((t) => t.value === flyer.template)?.label || flyer.template;
  ...
  // Upload a captured thumbnail to Cloudinary via the shared media route.
  async function uploadThumbnail(blob) {
    try {
      const fd = new FormData();
      fd.append("file", new File([blob], "flyer-thumb.png", { type: "image/png" }));
      ...
      return ""; // saving should still succeed without a thumbnail
    }
  }
  ...
  async function handlePDF() {
    ...
    try {
      ...
      pdf.addImage(img, "PNG", 0, 0, FLYER_W, FLYER_H);
  ...
  // Render the current flyer's content in each of the 4 templates and zip them.
  async function handlePack() {
    ...
    try {
      ...
      flushSync(() => setPackVariant(null));
    }
    if (failed) {
      setMsg(`Pack failed: ${failed.message}`);
      setBusy(null);
      return;
    }
    try {
  ...
  async function handleSaveLibrary() {
    ...
    try {
      ...
      const item = await saveFlyer({
        id: flyer.id,
        title: (flyer.title || "Untitled flyer").trim(),
  ...
  function handleNew() {
    setFlyer(defaultFlyer(flyer.template));
    setMsg("");
  }
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.6)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
            {templateLabel}
          </span>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={handleNew} style={toolBtn(false)}>
            + New
          </button>
          <button onClick={() => setShowSaved((v) => !v)} style={toolBtn(true)}>
            Saved ({flyers.length})
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        >
          <span style={{ fontSize: 10, fontWeight: 700, color: "#5A6485", letterSpacing: "0.12em", textTransform: "uppercase" }}>
            Live preview · {templateLabel} · 1080 × 1350
          </span>
          ...
          <div
            style={{
              ...
              flexShrink: 0,
              borderRadius: 12,
              boxShadow: "0 8px 32px rgba(5,17,31,0.18)",
              overflow: "hidden",
            }}
          >
            <div
          ...
          <p style={{ fontSize: 11, color: "#8892B0", textAlign: "center", margin: 0, maxWidth: 360 }}>
            Edit the form to update the preview live. <strong>Save to library</strong> stores the flyer
            (and a thumbnail) on the server; <strong>Export</strong> downloads a print-ready file.
          </p>
        </div>
      </div>

      {/* Offscreen native-size export target (html2canvas reads this) */}
      <div
        aria-hidden="true"
        style={{
          position: "fixed",
      ...
      {showSaved && (
        <>
          <div
            style={{ position: "fixed", inset: 0, background: "rgba(5,17,31,0.18)", zIndex: 99 }}
...
function toolBtn(solid) {
  return {
    ...
    cursor: "pointer",
    fontFamily: "'Lexend', sans-serif",
    background: solid ? "#E86A27" : "rgba(255,255,255,0.1)",
    color: solid ? "#fff" : "rgba(255,255,255,0.9)",
