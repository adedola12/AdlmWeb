...
export default function AdminRoles() {
  ...
  // Design Access is an alternative to the area matrix, not an addition to it.
  ...
  async function togglePerm(role, areaKey) {
    if (role.isSuperAdmin || role.designAccess) return; // all-on, locked
  ...
  async function createRole(e) {
    ...
    try {
      const res = await apiAuthed("/admin/roles", {
        token: accessToken,
        method: "POST",
        body: { name, permissions: newDesign ? [] : newPerms, designAccess: newDesign },
      });
      ...
      {loading ? (
        ...
        <>
              ...
              <tbody>
                {roles.map((role) => (
                  <tr key={role.key} className="border-t border-slate-100 dark:border-adlm-dark-border">
                    <td className="sticky left-0 bg-white dark:bg-adlm-dark-panel py-2 pr-3 align-middle">
                      <div className="flex items-center gap-2">
                        ...
                        {role.isSuperAdmin ? (
                          ...
                          <span
                            className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-adlm-blue-700/10 text-adlm-blue-700 dark:text-adlm-blue-400 ring-1 ring-adlm-blue-700/25"
                            title="Sees every admin section, but only ever placeholder data. Saving is simulated."
                          >
                            <FiShield className="w-3 h-3" /> design
                          </span>
                        ) : role.system ? (
                    ...
                    {staffAreas.map((a) => {
                      const locked = role.isSuperAdmin || role.designAccess;
                      const on = locked || role.permissions.includes(a.key);
                      return (
          ...
          <Reveal as="form" onSubmit={createRole} className="card mt-6" delay={60}>
            ...
            <label className="mt-3 flex items-start gap-2.5 rounded-xl border border-slate-200 dark:border-adlm-dark-border bg-slate-50 dark:bg-adlm-dark-raised p-3 cursor-pointer">
              <input
                type="checkbox"
                className="mt-0.5"
                checked={newDesign}
              ...
              <span className="min-w-0">
                <span className="text-sm font-medium flex items-center gap-1.5">
                  <FiShield className="w-3.5 h-3.5 text-adlm-blue-700" />
                  Design Access
                </span>
                <span className="block text-[11px] text-slate-500 dark:text-adlm-dark-muted leading-snug mt-0.5">
                  Opens every admin section for UI work, but the server replaces
                  all data with placeholders and simulates every save. No real
                  user, subscriber or revenue figure is ever shown. Replaces the
                  area list below.
                </span>
              </span>
            </label>

            <div className={`mt-3 ${newDesign ? "opacity-40 pointer-events-none" : ""}`}>
