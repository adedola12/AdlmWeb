...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-slate-900 dark:bg-adlm-dark-bg dark:text-adlm-dark-text">
      ...
      >
        {/* Background layers */}
        <div className="absolute inset-0 bg-[url('/hero-construction.jpg')] bg-cover bg-center scale-105" aria-hidden="true" />
        ...
        <div className="absolute inset-0 grid-overlay mask-radial opacity-60" aria-hidden="true" />

        {/* Decorative floating shapes — wrappers do pointer parallax,
            inner blobs do the idle float so the two transforms compose. */}
        <div className="absolute top-20 right-[10%]" aria-hidden="true" style={{ transform: "translate3d(calc(var(--px,0)*40px), calc(var(--py,0)*40px), 0)" }}>
          <div className="w-64 h-64 rounded-full opacity-[0.07] bg-adlm-blue-600 blur-3xl animate-float" />
        </div>
        <div className="absolute bottom-32 left-[5%]" aria-hidden="true" style={{ transform: "translate3d(calc(var(--px,0)*-55px), calc(var(--py,0)*-35px), 0)" }}>
          <div className="w-56 h-56 rounded-full opacity-[0.09] bg-adlm-orange blur-3xl animate-float-slow" />
        </div>
        <div className="absolute top-1/2 right-[28%]" aria-hidden="true" style={{ transform: "translate3d(calc(var(--px,0)*28px), calc(var(--py,0)*-28px), 0)" }}>
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              ...
              <div className="opacity-0 motion-safe:animate-[fade-in-up_700ms_ease-out_forwards]" style={{ animationDelay: "100ms" }}>
                ...
                >
                  ...
                  <span className="text-gradient-warm">Faster</span>,{" "}
                ...
                >
                  ...
                  >
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                  </Link>
              ...
              >
                {HERO_CARDS.map((card, idx) => (
                  ...
                  >
                    <div className="tilt-layer w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ backgroundColor: "rgba(232,106,39,0.15)" }}>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-2 sm:grid-cols-4 rounded-2xl glass-dark shadow-depth-lg overflow-hidden divide-x divide-white/10 [&>*:nth-child(n+3)]:border-t [&>*:nth-child(n+3)]:border-white/10 sm:[&>*]:border-t-0">
              {[
                ...
                >
                  <span className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgba(35,156,255,0.15)" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-adlm-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={stat.icon} /></svg>
                  </span>
                  <span>
                    <span className="block text-lg sm:text-xl font-bold text-white">{stat.node}</span>
                    <span className="block text-[11px] sm:text-xs text-white/60">{stat.label}</span>
                  </span>
                </button>
              ))}
            </div>
      ...
      <section className="relative mx-auto max-w-6xl px-4 sm:px-6 pt-20 sm:pt-24 pb-12 sm:pb-16">
        <Reveal className="text-center">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold text-adlm-orange bg-adlm-orange/10 border border-adlm-orange/20">
            Why ADLM
          </span>
          <h2 className="mt-4 text-2xl sm:text-3xl font-bold tracking-tight text-adlm-navy dark:text-white">
        ...
        <div className="mt-12 grid gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-6 lg:auto-rows-fr">
          <BentoCard
            className="lg:col-span-3"
            delay={0}
            accent="orange"
            title="Everything You Need in One Place"
            text="No need to jump between software. Access RateGen, Revit & PlanSwift plugins, training, certifications, and reporting — all with one login."
            icon={<path d="M3 7h18v10H3zM7 7V3h10v4" />}
          />
          <BentoCard
            className="lg:col-span-3"
            delay={80}
            ...
            text="Get accurate material and labour rates tailored to each geopolitical zone. Every rate comes with history, updates, and audit trails you can defend."
          ...
          <BentoCard
            ...
            text="Manage team roles, learning progress, and project permissions. Keep everything synced in the cloud so your team stays organized."
            icon={<><path d="M3 21v-6a4 4 0 014-4h10a4 4 0 014 4v6" /><circle cx="7.5" cy="7" r="3" /><circle cx="16.5" cy="7" r="3" /></>}
          />
          <BentoCard
            ...
            text="Export BoQs, reports, and logs you can proudly present to clients, auditors, or management — defensible numbers, every time."
            icon={<><path d="M12 2l7 4v6c0 5-3 8-7 10-4-2-7-5-7-10V6l7-4z" /><path d="M9 12l2 2 4-4" /></>}
          />
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-adlm-navy text-white">
        <div className="absolute inset-0 grid-overlay mask-radial opacity-50" aria-hidden="true" />
        <div className="absolute -top-24 left-1/3 w-96 h-96 rounded-full bg-adlm-blue-600/20 blur-3xl animate-float" aria-hidden="true" />
        <div className="absolute -bottom-24 right-1/4 w-80 h-80 rounded-full bg-adlm-orange/20 blur-3xl animate-float-slow" aria-hidden="true" />
...
function BentoCard({ icon, title, text, accent = "orange", delay = 0, className = "" }) {
