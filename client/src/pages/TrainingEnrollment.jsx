// src/pages/TrainingEnrollment.jsx
...
export default function TrainingEnrollment() {
  ...
  async function load() {
    ...
    setErr("");
    try {
      const { data } = await apiAuthed.get(`/me/trainings/${enrollmentId}`);
      setE(data);
    } catch (x) {
  ...
  // If paystack redirected here, try verify using stored reference
  useEffect(() => {
    (async () => {
      if (!verify) return;
      if (!e?.payment?.reference) return;
      if (e?.payment?.paid) return;

      try {
        await apiAuthed.get(`/trainings/paystack/verify`, {
  ...
  async function submitForm() {
    ...
    setErr("");
    try {
      await apiAuthed.post(`/me/trainings/${enrollmentId}/form`, form);
      await load();
    } catch (x) {
      setErr(x?.response?.data?.error || x.message || "Failed");
    } finally {
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-2xl border bg-white">
          ...
          <div className="mt-1 font-bold">
            {e.formSubmittedAt ? "Submitted" : paid ? "Pending" : "Locked"}
          </div>
          <div className="text-sm text-gray-600 mt-2">
            {e.formSubmittedAt
        ...
        <div className="mt-8 bg-white border rounded-2xl p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <a
                className="px-4 py-2 rounded-xl border font-semibold hover:bg-gray-50"
                href={`/me/trainings/${enrollmentId}/ics`}
              >
                Download .ICS
              </a>
            </div>
