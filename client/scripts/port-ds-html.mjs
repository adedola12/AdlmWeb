...
function component({ name, jsx, usesLink, from, wrap, takesData, usesPromo }) {
  const imports = ['import React from "react";'];
  if (usesLink) imports.push('import { Link } from "react-router-dom";');
  if (usesPromo) imports.push('import DsPromoLive from "../DsPromoLive.jsx";');
  const open = wrap ? `<div className="ds">` : `<>`;
  const close = wrap ? `</div>` : `</>`;
  const args = takesData ? "{ d }" : "";
  // The injected promo token renders as {DsPromoSlot}. Declaring it here is
  // what keeps the token free of angle brackets — htmlToJsx scans for `<` and
  // would parse them as markup, lowercasing the tag, so `<DsPromo />` came out
  // as `<dspromo />`: an unknown element that renders nothing at all.
  const slot = usesPromo ? "  const DsPromoSlot = <DsPromoLive />;\n" : "";
